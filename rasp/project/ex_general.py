...
if len(sys.argv)<2:
    ...
    print("usage: python ex.py <id_analysis>")
...
def determinar_tipo_suelo(promedio_ph):
    ...
    if promedio_ph < 7:
        ...
        return "Acido", "Dificultad de desarrollo de la mayori­a de los cultivos, a su vez hay dificultad de retencion de muchos nutrientes."

    elif promedio_ph == 7:

        return "Neutro", "Optimo para los cultivos."

    else:

        return "Basico", "Dificultad de desarrollo de la mayoria de los cultivos, posible aparicion de clorosis ferrica."
...
cursor.execute('SELECT humidity, temperature, conductivity, pH, nitrogen, phosphorus, potassium FROM data where id_analysis = ?', (id_analysis,))
...
for row in rows:
    ...
    datos["Nitrogeno"].append(extract_number(nitrogen))

    datos["Fosforo"].append(extract_number(phosphorus))
...
cursor.execute('''
    ...
    INSERT INTO data_analysis (id_analysis, humidity, temperature, conductivity, pH, nitrogen, phosphorus, potassium)
...
for cultivo, detalles in resultados.items():
    ...
    if detalles:
        ...
        cursor.execute('INSERT INTO resultados_analysis (id_analysis, cultivo, apto, detalles, porcentaje) VALUES (?, ?, ?, ?, ?)', 

                       (id_analysis, cultivo, False, detalles_str, porcentajes[cultivo]))
        ...
        cursor.execute('INSERT INTO resultados_analysis (id_analysis, cultivo, apto, detalles, porcentaje) VALUES (?, ?, ?, ?, ?)', 
                       ...
                       (id_analysis, cultivo, True, detalles_str, porcentajes[cultivo]))
...
cursor.execute('INSERT INTO resultados_analysis (id_analysis, cultivo, apto, detalles, porcentaje) VALUES (?, ?, ?, ?, ?)', 
               ...
               (id_analysis, 'General', False, 'Porcentaje de cultivos no aptos', porcentaje_no_aptos))



cursor.execute('INSERT INTO resultados_analysis (id_analysis, cultivo, apto, detalles, porcentaje) VALUES (?, ?, ?, ?, ?)', 

               (id_analysis, tipo_suelo, False, mensaje_suelo, promedio_ph))



cursor.execute('INSERT INTO resultados_analysis (id_analysis, cultivo, apto, detalles, porcentaje) VALUES (?, ?, ?, ?, ?)', 

               (id_analysis, 'Probabilidad de Suelo Neutro', False, 'Probabilidad de que el suelo sea neutro', probabilidad_neutro))
